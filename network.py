...
class East:
    ...
    def __init__(self):
        self.input_img = Input(name='input_img',
                               shape=(None, None, cfg.num_channels),#输入数据尺寸为[H, W, C]
                               dtype='float32')
        # resnet = ResNet50(input_tensor= self.input_img,
        #                   weights= 'imagenet',
        #                   include_top=False)
        vgg16 = VGG16(input_tensor=self.input_img,#VGG conv部分提取特征
                      weights='imagenet',
                      include_top=False)
        if cfg.locked_layers:
            # locked first two conv layers
            locked_layers = [vgg16.get_layer('block1_conv1'),#固定前几层梯度不更新
                             vgg16.get_layer('block1_conv2')]
            for layer in locked_layers:
                layer.trainable = False
        self.f = [vgg16.get_layer('block%d_pool' % i).output#找到各个阶段的输出作为fpn层级金字塔的输入
    ...
    def east_network(self):
        ...
        east_detect = Concatenate(axis=-1,
                                  name='east_detect')([inside_score,#concat作为输出
                                                       side_v_code,
                                                       side_v_coord])
