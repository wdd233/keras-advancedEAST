...
def quad_loss(y_true, y_pred):
    ...
    logits = y_pred[:, :, :, :1]#[batch, H, W, C]数据格式
    labels = y_true[:, :, :, :1]
    # balance positive and negative samples in an image
    beta = 1 - tf.reduce_mean(labels)#计算平衡系数\beta
    # first apply sigmoid activation
    predicts = tf.nn.sigmoid(logits)#前三个值使用sigmoid输出
    # log +epsilon for stable cal
    inside_score_loss = tf.reduce_mean(
        -1 * (beta * labels * tf.log(predicts + cfg.epsilon) +
              (1 - beta) * (1 - labels) * tf.log(1 - predicts + cfg.epsilon)))#此处对照EAST中的L_s公式
    ...
    side_vertex_code_loss = \
        tf.reduce_sum(tf.reduce_sum(pos + neg, axis=-1) * positive_weights) / (#code_loss
    ...
    g_hat = y_pred[:, :, :, 3:]#[3:]是四个box坐标
    g_true = y_true[:, :, :, 3:]
    vertex_weights = tf.cast(tf.equal(y_true[:, :, :, 1], 1), tf.float32)#正样本权重1
    ...
    return inside_score_loss + side_vertex_code_loss + side_vertex_coord_loss#loss 一共有三部分构成
